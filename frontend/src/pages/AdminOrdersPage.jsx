...
export default function AdminOrdersPage() {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><Link className="admin-brand" to="/admin/products"><span>V</span><div>VIRTUAL<strong>TRY-ON</strong></div></Link><nav><small>WORKSPACE</small><Link to="/admin/products"><ShoppingBag size={19}/> Products</Link><Link className="is-active" to="/admin/orders"><Box size={19}/> Orders <b>{orders.length}</b></Link><Link to="/admin/customers"><Users size={19}/> Customers</Link><small>MANAGE</small><Link to="/admin/promotions"><PackageCheck size={19}/> Promotions</Link></nav><div className="admin-account"><div>{user?.fullName?.slice(0,2).toUpperCase()||'AD'}</div><span><strong>{user?.fullName}</strong><small>{user?.email}</small></span></div></aside>
