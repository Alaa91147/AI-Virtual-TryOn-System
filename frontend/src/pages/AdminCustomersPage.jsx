...
import { Bell, Box, ChevronDown, Eye, MailCheck, PackageCheck, RefreshCw, Search, ShoppingBag, UserCheck, Users } from 'lucide-react';
...
export default function AdminCustomersPage() {
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar"><Link className="admin-brand" to="/admin/products"><span>V</span><div>VIRTUAL<strong>TRY-ON</strong></div></Link><nav><small>WORKSPACE</small><Link to="/admin/products"><ShoppingBag size={19}/> Products</Link><Link to="/admin/orders"><Box size={19}/> Orders</Link><Link className="is-active" to="/admin/customers"><Users size={19}/> Customers <b>{customerAccounts.length}</b></Link><small>MANAGE</small><Link to="/admin/promotions"><PackageCheck size={19}/> Promotions</Link></nav><div className="admin-account"><div>{user?.fullName?.slice(0,2).toUpperCase() || 'AD'}</div><span><strong>{user?.fullName}</strong><small>{user?.email}</small></span></div></aside>
    <main className="admin-main"><header className="admin-topbar"><div><span>Community</span><b>/</b><strong>Customers</strong></div></header><div className="admin-content">
      <section className="admin-page-title"><div><span className="admin-eyebrow">CUSTOMER MANAGEMENT</span><h1>Customers</h1><p>Understand your audience, review activity, and manage customer access.</p></div></section>
...
function NotificationModal({ customer, token, onClose, onError }) {
  ...
  return <div className="customer-modal-backdrop" onMouseDown={onClose}><form className="customer-modal" onSubmit={submit} onMouseDown={(e)=>e.stopPropagation()}><span className="admin-eyebrow">CUSTOMER MESSAGE</span><h2>Notify {customer.fullName}</h2><p>The message will appear in this customer’s notification center.</p><label>Title<input value={title} maxLength="100" onChange={(e)=>setTitle(e.target.value)} required/></label><label>Message<textarea value={message} maxLength="500" rows="5" onChange={(e)=>setMessage(e.target.value)} required placeholder="Write your message..."/></label><div><button type="button" className="admin-secondary-btn" onClick={onClose}>Cancel</button><button className="admin-primary-btn" disabled={sending}>{sending?'Sending...':'Send notification'}</button></div></form></div>;
}
